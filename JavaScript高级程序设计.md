...
总结:
            1. 不使用 var
               ...
               2. const 优先，let 次之
                     运行时强制保持变量不变，也可以让静态代码分析工具提前发现不合法的赋值操作。因此，很多开发者认为应该优先使用 const 来声明变量,只在提前知道未来会有修改时，再使用 let。这样可以让开发者更有信心地推断某些变量的值永远不会变，同时也能迅速发现因意外赋值导致的非预期行为。



## 数据类型

简单数据类型(原始数据类型): undefined、Null、Boolean、Number、String、Symbol
...
- 声明了变量但未初始化, 保存的是 undefined

- 包含了 undefined 的值和未定义变量是是不同的意思

- 对未初始化的变量和未生命的变量调用 typeof 返回的结果都是 undefined

  ps: 建议在声明的变量时进行初始化. 这样 typeof 返回 undefined 时, 就可以知道是变量未声明, 而不是声明了没有初始化
